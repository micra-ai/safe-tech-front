  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <select
          className="rounded-md border border-gray-200 bg-gray-50 px-2 py-1 text-sm text-gray-700 focus:border-blue-400 focus:bg-white focus:outline-none"
          value={dia}
      ...
      {loading ? (
        <div className="flex h-40 items-center justify-center text-gray-500 text-sm">Cargando…</div>
      ) : err ? (
        <div className="flex h-40 items-center justify-center text-red-600 text-sm">{err}</div>
      ) : !dia ? (
        <div className="flex h-40 items-center justify-center text-gray-500 text-sm">Selecciona un día.</div>
      ) : frames.length === 0 ? (
        <div className="flex h-40 items-center justify-center text-gray-500 text-sm">Sin detecciones.</div>
      ) : (
        <>
          <div className="relative overflow-hidden rounded-xl">
            <img
              src={current}
              alt={`Frame ${idx + 1}`}
              className="block w-full max-h-[200px] object-cover"
            />
            {hhmmss && (
